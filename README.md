...
<img src="docs/img/demo.gif" alt="GITMIR Codex Control — demo" width="920">

<sub>Pick a project → <b>▶ Run Codex</b> → copy the <code>gitmir-model</code> skill → open <b>Model</b> and see the product's business logic and data flows — laid out by <a href="https://github.com/kieler/elkjs">ELK</a> in the GITMIR HUD.</sub>
...
```bash
git clone https://github.com/gitmir-hello/gitmir-codex-control.git
cd gitmir-codex-control
node server.ts
```

Opens on **http://localhost:4599**. Add a project folder → **▶ Run Codex** →
paste the **`gitmir-model`** skill. Codex reads the repo and writes
...
`AGENTS.md`, so every later session reads the model first and refreshes it after
changing code.
...
- **All your projects on one screen.** Add any folder on any disk; launch `codex`
...
- **Tasks** — a live log of what Codex did in this project.
- **Preview** — open any URL, click an element on the page, get a prompt naming it
  and the files it probably lives in.
- **Skills** — reusable instructions you copy into Codex with one click.
...
| **Tasks — what Codex did** | **Order state machine (fullscreen)** |
...
From there: **📋 copy the context** into Codex, or **＋ create a task** — a plain
...
**1. Build the model.** Paste **`gitmir-model`** into a Codex session in that
...
it looks like you documented something. If any of that is missing, ask Codex to
...
| **GITMIR Codex Control** | **yes, at the product level** | **yes, explicitly** | **yes, and records what happened** |
...
Copy from the dashboard (**Settings → 📋 skill**) and paste into your Codex
...
| **`task-log`** | Keeps a human-readable log of what Codex completed, shown in the **Tasks** tab. |
...
This runs **entirely on your computer**. It launches Codex locally, builds the
...
- The Codex CLI (`codex`) on your `PATH`
...
**GITMIR Codex Control** shortcut on your Desktop that starts the dashboard with
...
**GITMIR Codex Control** app on your Desktop that starts the dashboard *in the
...
`~/Library/Logs/gitmir-codex-control.log`.
...
We built this for ourselves. We run Codex all day across dozens of projects,
...
machine?](https://github.com/gitmir-hello/gitmir-codex-control/discussions/1)** is
