// GITMIR Codex Control — local dashboard for running Codex across projects.
...
async function chooseFolder(): Promise<string | null> {
  ...
  if (plat === 'darwin') {
    const script =
      'try\n' +
      '  set f to choose folder with prompt "Choose a project folder for Codex"\n' +
  ...
  if (plat === 'win32') {
    const ps =
      ...
      "$f = $a.BrowseForFolder(0, 'Choose a project folder for Codex', 0x51, 0); " +   // BIF_RETURNONLYFSDIRS|BIF_EDITBOX|BIF_NEWDIALOGSTYLE
  ...
  const tools = [
    ['zenity', ['--file-selection', '--directory', '--title=Choose a project folder for Codex']],
...
// Open a terminal in the folder and run `codex` — detects the OS.
function openInTerminal(projectPath: string) {
  ...
  if (plat === 'darwin') {
    const script =
      ...
      '    activate\n' +
      '    do script "cd " & quoted form of p & " && codex"\n' +
      '  end tell\n' +
      'end run';
    return osascript(script, [projectPath]);
  }

  // Windows — new console window in the project dir running codex, kept open (/k).
  // `start "" /D <dir> cmd /k codex` avoids cd/quoting issues; Node quotes the path.
  if (plat === 'win32') {
    return new Promise((resolve, reject) => {
      const child = spawn('cmd.exe',
        ['/c', 'start', 'GITMIR Codex', '/D', projectPath, 'cmd', '/k', 'codex'],
  ...
  if (plat === 'linux') {
    return new Promise((resolve, reject) => {
      const inner = 'cd ' + JSON.stringify(projectPath) + ' && codex; exec bash';
      const candidates = [
...
async function previewFetch(rawUrl: string, allowLoopback: boolean): Promise<PreviewResult> {
  ...
  for (;;) {
    ...
    try {
      r = await fetch(u.href, { redirect: 'manual', signal: ctl.signal, headers: { 'user-agent': 'GitMirCodexControl/preview', accept: 'text/html,*/*' } });
    } catch (e) {
      clearTimeout(timer);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/tasks') {
      const p = url.searchParams.get('path') || '';
      const file = path.join(p, '.codex', 'tasks.json');
      try {
      ...
      if (!who && exists) {
        try {
          ...
          const SKIP = new Set(['node_modules', '.git', 'dist', 'build', '.next', 'out', 'vendor',
            'coverage', '.cache', '.venv', '__pycache__', '.gitmir', 'tasks', 'docs', '.codex']);
...
<title>GITMIR Codex Control</title>
...
<body>
  <aside class="side">
    <div class="side-top">
      <div class="brand"><a class="brand-link" href="https://ide.gitmir.com" target="_blank" rel="noopener" title="Open ide.gitmir.com"><img class="brand-logo" src="/vendor/gitmir-wordmark.svg" alt="GitMir IDE" draggable="false"></a><span class="brand-sub">Codex Control</span><span class="c" id="count"></span></div>
      <button class="add" id="addBtn">＋ Add project</button>
      <input class="search" id="search" placeholder="Search…" autocomplete="off">
    </div>
    <div class="list" id="list"></div>
    <div class="side-foot">
      <a class="brand-src" href="https://github.com/gitmir-hello/gitmir-codex-control" target="_blank" rel="noopener">Source</a>
    ...
    <div class="placeholder" id="placeholder">
      ...
      <ol class="ph-steps">
        ...
        <li>In <b>Settings</b> copy <b>📋 gitmir-model</b> and paste it into that session (⌘V + Enter).
            Codex reads the real code and writes <code>.gitmir/model/</code> — then the <b>Model</b> tab fills
