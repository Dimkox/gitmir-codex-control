...
name: task-runner
description: >-
  Execute the file-based task queue under tasks/ until empty: move each task
  todo → inprogress → verify → done, run real acceptance checks, never mark
  done on "looks right". Use when the user says run the queue, process tasks,
  or execute pending work.
---
