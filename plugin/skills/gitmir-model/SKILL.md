...
description: >-
  ...
  installs a mandatory AGENTS.md rule so the model stays the product's living source
...
Ensure the project root has an `AGENTS.md` that contains the section below. Create
`AGENTS.md` if it is absent; if it already exists, add or update **only** this
...
With that rule in `AGENTS.md`, the model becomes a **living source of truth**: each
...
- Write **only** under `.gitmir/` — plus the single "GitMir model — source of
  truth" section in the project's `AGENTS.md` (see the Standing rule section). Do
