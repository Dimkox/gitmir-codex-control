@echo off
REM Puts a "GITMIR Codex Control" shortcut on your Desktop (Windows).
REM
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$dir='%DIR%'; $lnk = Join-Path ([Environment]::GetFolderPath('Desktop')) 'GITMIR Codex Control.lnk'; $w = New-Object -ComObject WScript.Shell; $s = $w.CreateShortcut($lnk); $s.TargetPath = (Get-Command powershell).Source; $s.Arguments = '-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"' + (Join-Path $dir 'launch.ps1') + '\"'; $s.WorkingDirectory = $dir; $s.Description = 'Start GITMIR Codex Control and open it in the browser'; $ico = Join-Path $dir 'vendor\gitmir.ico'; if (Test-Path $ico) { $s.IconLocation = $ico }; $s.WindowStyle = 7; $s.Save(); Write-Host ('Shortcut created: ' + $lnk)"

echo.
echo Double-click "GITMIR Codex Control" on your Desktop: the dashboard starts with no
