...
**Short version:** GITMIR Codex Control runs entirely on your machine and makes
...
1. **The dashboard ↔ your browser — `localhost` only.** The tool serves a local
   ...
   interface.
2. **Codex — talks to Anthropic under *your* account.** This tool's job is
   to *launch* `codex` in your project folder. Once running, Codex
   communicates with Anthropic using your own credentials/subscription — exactly
   as it does when you run `codex` yourself in a terminal. **This tool adds
   nothing to that and sees none of it.** If you trust Codex, nothing here
...
2. **Run it air-gapped.** Disconnect from the network and start it. The dashboard,
   the model view, the task log — all keep working. (Only *launching Codex* needs
   the network, because Codex talks to Anthropic — see above.)
3. **Watch outbound connections.** Point Little Snitch / `lsof -i` / `tcpdump` at
   the process. You will see loopback traffic and — only if you launch Codex —
   connections from `codex` to Anthropic. Nothing to us.
...
- **The product model:** `.gitmir/model/*.json` inside each of *your* project
  folders.
- **Task log / queue:** `.codex/tasks.json` and `tasks/` inside your projects.
...
- **Incoming data from teammates is sandboxed.** A model snapshot arriving over the
  ...
  by itself; your local Codex only acts on it when you run the queue.
...
the sections above describe: your machine, and nothing outbound but Codex.
