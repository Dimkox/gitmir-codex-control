...
function renderDetail(){
  ...
  if (!p){
    ...
    mainEl.innerHTML =
      ...
      '<ol class="ph-steps">' +
        ...
        '<li><b>▶ Run Codex</b> — opens a terminal in that folder with <code>codex</code> running.</li>' +
        '<li>In <b>Settings</b> copy <b>📋 gitmir-model</b> and paste it into that session (⌘V + Enter). ' +
          'Codex reads the real code and writes <code>.gitmir/model/</code> — then the <b>Model</b> tab fills with ' +
  ...
  wrap.innerHTML =
    ...
    '<div class="pane" data-pane="settings">' +
      ...
      '<div class="actions">' +
        '<button class="run" id="runBtn">▶ Run Codex</button>' +
        '<button class="ghost" id="finderBtn">🗂 Finder</button>' +
        '<button class="del" id="delBtn">🗑 Remove</button>' +
      '</div>' +
      '<div class="skills-box">' +
        '<div class="skills-label">Skills — copy and paste into codex (⌘V + Enter)</div>' +
        '<div class="skills-btns" id="skillsBtns"></div>' +
      '</div>' +
    '</div>' +
    '<div class="pane" data-pane="tasks">' +
      '<div class="tasks-head"><span class="t">What Codex did</span><span class="upd" id="taskUpd"></span></div>' +
      '<div id="taskList"></div>' +
    '</div>' +
...
function renderSkillButtons(){
  ...
  for(const s of SKILLS){
    ...
    it.className = 'skill-item'; it.title = 'Copy — paste into Codex';
    it.innerHTML =
...
async function copySkill(name, title){
  try{
    ...
    toast('Copied: '+(title||name)+' ✓  Paste into codex (⌘V) and Enter');
    ...
    view.innerHTML = modelSrc
      ...
      : '<div class="model-empty"><b>This project has no model of its own yet.</b><br>'+
        (shared.length
          ...
          : 'In the <b>Settings</b> tab click <b>📋 gitmir-model</b>, paste into codex (⌘V + Enter) — it will build <code>.gitmir/model/</code>, and diagrams of data, processes and flows will appear here.')+'</div>';
    return;
  }
...
function renderModelStale(d){
  ...
  box.innerHTML='<div class="stale-hd">⚠ This model is older than the code</div>'+
    ...
    '<button class="run stale-fix">📋 Copy gitmir-model — paste into Codex to refresh</button>';
...
function openContextPopup(kind,id){
  ...
  ov.innerHTML=
    '<div class="ctx-modal">'+
      ...
      '<div class="ctx-note">'+(modelSrc
        ...
        : 'Deterministic context — assembled from the model by walking id-links. Paste into Codex, or turn it into a queued task.')+'</div>'+
  ...
  // otherwise the local Codex would read it as a description of THIS project.
...
async function refreshTasks(pathStr){
  ...
  if(!tasks.length){
    cont.innerHTML = '<div class="tasks-empty">No entries yet.<br>1) <b>▶ Run Codex</b> · 2) in Settings click <b>📋 task-log</b> · 3) paste into codex (⌘V) and Enter — it will start logging what it does here.</div>';
    return;
  }
...
// Picking an element builds the text you paste into Codex and puts it straight
...
function pvText(d, hits, model, what){
  ...
  L.push((what && what.trim()) ? what.trim() : '<describe it here, or just type it to Codex after pasting>');
  return L.join('\n');
}
...
async function pvRenderPicked(d){
  ...
  ov.innerHTML=
    '<div class="ctx-modal">'+
      ...
      '<div class="ctx-note">'+(copied
        ? '✓ Copied to the clipboard — paste it into Codex (⌘V + Enter) and it will know which element you mean.'
        : 'Press <b>Copy</b> below, then paste it into Codex (⌘V + Enter).')+'</div>'+
  ...
  ov.querySelector('.pv-copy').addEventListener('click', async ()=>{
    try{ await copyToClipboard(current()); toast('Copied ✓  paste into Codex'); }catch{ toast('Could not copy — select the text and copy it', true); }
  });
