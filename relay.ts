...
// incoming tasks are written to the project's tasks/todo/ (so your local Codex
...
// is still inside the destination — otherwise "../../../.codex/settings.json"
...
async function shareView(opts?: { name?: string; expiresIn?: number | null; access?: string }): Promise<ActionResult & { url?: string }> {
  ...
  try {
    const r = await fetch(endpoint, {
      ...
      headers: { 'content-type': 'application/json', 'x-gitmir-key': state.key, 'user-agent': 'GitMirCodexControl/share' },
      body: JSON.stringify({
